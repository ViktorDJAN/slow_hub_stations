...
//        ConfigsFile.setConfigsBasedOnJSONFile("slow_charging_station/home/root/chargingStation/configurations/configuration.json");
        ...
        ScheduledExecutorService scheduled_service = Executors.newScheduledThreadPool(6 + ConfigsFile.mode3_controller_addresses.size());
        ...
        Executors.newSingleThreadScheduledExecutor().scheduleAtFixedRate(chargePointOcpp::connect, 20, 3000, TimeUnit.MILLISECONDS);
        ...
        Executors.newSingleThreadScheduledExecutor().scheduleAtFixedRate(new CalculatingAvailableParamsProcess(ConfigsFile.outer_en_meter_id, EnergyMeterType.OUTER_ENERGY_METER, EnergyMeterKind.NON_BUILT_IN), 20, 650, TimeUnit.MILLISECONDS);
        // KICK OFF TRACKER FOR TRACKING CURRENT CHANGES
        Executors.newSingleThreadScheduledExecutor().scheduleAtFixedRate(new PhaseCurrentChangesTracker(ConfigsFile.outer_en_meter_id, EnergyMeterType.OUTER_ENERGY_METER), 400, 700, TimeUnit.MILLISECONDS);

        // OCPP_HANDLER_START
        Executors.newSingleThreadScheduledExecutor().execute(new OcppProxyHandler(chargePointOcpp));
        ...
        for (String address : ConfigsFile.mode3_controller_addresses) {
            ...
            Executors.newSingleThreadScheduledExecutor().scheduleAtFixedRate(new CalculatingAvailableParamsProcess(embeddedEnMeterId, EnergyMeterType.INNER_ENERGY_METER, EnergyMeterKind.BUILT_IN), 20, 1500, TimeUnit.MILLISECONDS);
            ...
            service.schedule(new StationRunProcess(station, chargePointOcpp),3000, TimeUnit.MILLISECONDS);
        }
